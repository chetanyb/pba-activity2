...
fn ungroup(_blocks: Vec<[u8; BLOCK_SIZE]>) -> Vec<u8> {
	todo!()
}

/// Does the opposite of the pad function.
fn unpad(_data: Vec<u8>) -> Vec<u8> {
	todo!()
}
...
fn cbc_encrypt(plain_text: Vec<u8>, key: [u8; BLOCK_SIZE]) -> Vec<u8> {
    ...
    let encrypted_data = ungroup(encrypted_blocks);
    encrypted_data
}
...
fn cbc_decrypt(cipher_text: Vec<u8>, key: [u8; BLOCK_SIZE]) -> Vec<u8> {
    ...
    let decrypted_groups = ungroup(decrypted_blocks);
    unpad(decrypted_groups)
}
...
fn ctr_decrypt(cipher_text: Vec<u8>, key: [u8; BLOCK_SIZE]) -> Vec<u8> {
    ...
    let decrypted_data = ungroup(decrypted_blocks);
